...
const sections = [
  ...
  { id: 'auth', title: 'Auth' },
  { id: 'form', title: 'Form System' },
  { id: 'email', title: 'Email System' },
      ...
      <div className="container mx-auto max-w-5xl px-6">
        <header className="space-y-6 pt-10 text-center">
          ...
          >
            ...
            <p className="mx-auto max-w-2xl text-lg leading-relaxed text-muted-foreground">
              A powerful multi-tenant Content Management System designed for
              speed, isolation, and scalability.
            </p>
        ...
        <section id="overview" className="scroll-mt-32 pt-20">
          <SectionHeader
            num={1}
            title="What is Elly?"
            subtitle="Overview and four pillars"
          />
          <div className="space-y-8">
            <p className="leading-relaxed text-muted-foreground">
              Elly is a headless CMS and rapid MVP platform built on Spring Boot
              3.5. Pages, components, dynamic forms, asynchronous email and user
              authentication — all isolated per tenant.
            </p>
        ...
        <section id="auth" className="scroll-mt-32">
          <SectionHeader
            ...
            subtitle="JWT flow, OAuth2, and refresh tokens"
          />
              ...
              <ol className="space-y-3">
                {[
                  ...
                  },
                  {
                    ...
                    desc: 'Validate · revoke check · issue new Access',
                  },
                ].map((s, i) => (
                  <li
                    key={i}
                ...
                {[
                  {
                    ...
                    desc: 'Create user · assign role · async verification email',
                  },
                  {
                    code: 'OAuth2 Providers',
                    desc: 'Google · Facebook · GitHub → SuccessHandler → JWT',
                  },
                  {
                    code: 'Redis Auth Cache',
                    desc: 'auth:user:{username} · TTL 30m · 3 SQL → 1 GET',
                  },
                ].map((s, i) => (
                  <li
                    key={i}
          ...
          <div className="border-border/50 mt-6 overflow-hidden rounded-xl border">
            <table className="w-full text-left text-sm text-white">
              ...
              <tbody className="divide-border/50 divide-y">
                {[
                  { f: 'token', d: 'UUID · unique per session' },
                  { f: 'expiryDate', d: 'LocalDateTime' },
                  { f: 'revoked', d: 'Boolean · logout flag' },
        ...
        <section id="form" className="scroll-mt-32">
          <SectionHeader
            num={3}
            title="Form System"
            subtitle="JSONB schema, validation strategy, conditional fields"
          />
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <Card className="border-border/50 bg-muted/20 p-6">
              <h3 className="mb-4 font-mono text-xs uppercase tracking-widest text-violet-400">
              ...
              <ul className="space-y-2 text-[11px]">
                {[
                  ...
                  ['schema', 'JSONB → FormSchema (fields + config)'],
              ...
              <ul className="space-y-2 text-[11px]">
                {[
                  ...
                  ['condition', 'ConditionRule: field · operator · value'],
                  ['operators', 'EQUALS · NOT_EQUALS · GT · LT'],
                ].map(([k, v]) => (
                  <li key={k} className="flex gap-3">
            ...
            <div className="space-y-6">
              <Card className="border-border/50 bg-muted/20 p-6">
                ...
                <div className="space-y-2">
                  {[
                    ['TextFieldValidator', 'min/max length, regex'],
                    ['NumberFieldValidator', 'range, integer check'],
                    ['SelectFieldValidator', 'allowed values'],
                  ].map(([k, v]) => (
                    <div
                      key={k}
              ...
              <Card className="border-border/50 bg-muted/20 p-6">
                ...
                <ol className="space-y-2 text-[11px] text-muted-foreground">
                  {[
                    'Fetch FormDefinition',
                    'Iterate fields → ConditionEvaluator',
                    'Hidden → strip key from payload',
                    'Visible → run FieldValidator strategy',
                    'Cleaned → save FormSubmission (JSONB)',
                  ].map((s, i) => (
        ...
        <section id="email" className="scroll-mt-32">
          <SectionHeader
            num={4}
            title="Email System"
            subtitle="Async messaging, retry & DLQ"
          />
          <Card className="border-border/50 bg-muted/20 mb-6 p-6">
            ...
            <ol className="space-y-2 text-[11px]">
              {[
                ...
                [
                  'EmailLog PENDING',
                  'Persisted to DB · 202 Accepted returned immediately',
                  'text-violet-400',
                ],
                [
                  'RabbitMQ email-queue',
                  'Only emailLogId is published',
                  'text-cyan-400',
                ],
                [
                  'Consumer',
                  'Fetch log → Thymeleaf render → TenantMailSenderFactory → Gmail SMTP',
                  'text-cyan-400',
                ],
                [
                  'Success / Failure',
                  'SENT · retry-queue TTL 30s · max 3× → FAILED + DLQ',
                  'text-pink-400',
                ],
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <Card className="border-border/50 bg-muted/20 p-6">
              ...
              <p className="text-[11px] leading-relaxed text-muted-foreground">
                <strong className="text-white">EmailLog fields:</strong>{' '}
              ...
              <p className="mt-3 text-[11px] leading-relaxed text-muted-foreground">
                ...
                @Scheduled every 5 minutes · finds PENDING records older than 5
                minutes · re-queues batches of 50.
              </p>
            </Card>
          </div>
        </section>
            ...
            {[
              {
                num: 1,
                name: 'IController',
                desc: 'Interface contracts, endpoint signatures',
                tags: ['@RestController'],
              },
              {
                num: 2,
                name: 'Controller',
                desc: 'HTTP layer, request parsing, response wrapping',
                tags: ['RootEntityResponse<T>'],
              },
              {
                num: 3,
                name: 'IService → Service',
                desc: 'Business logic, caching, tenant context',
                tags: ['@Cacheable', '@Transactional'],
              },
              {
                num: 4,
                name: 'Repository',
                desc: 'Spring Data JPA, Entity Graph queries',
                tags: ['@EntityGraph'],
              },
              {
                num: 5,
                name: 'Entity + DTO',
                desc: 'MapStruct mapping, Entity never exposed via API',
        ...
        <section id="tenancy" className="scroll-mt-32">
          <SectionHeader
            num={6}
            title="Multi-Tenancy"
            subtitle="Database-per-tenant isolation"
          />
            ...
            {[
              {
                ...
                desc: 'Each tenant has its own PostgreSQL database. Cross-tenant data leaks are impossible.',
                color: 'text-violet-400',
              },
              {
                icon: ServerIcon,
                title: 'ThreadLocal Context',
                desc: 'TenantContext uses ThreadLocal for request-scoped isolation; guaranteed cleanup in finally blocks.',
                color: 'text-cyan-400',
              },
              {
                icon: DatabaseIcon,
                title: 'HikariCP Pools',
                desc: 'Each tenant DB has its own optimized HikariCP pool — no resource contention.',
                color: 'text-amber-400',
              },
              {
                icon: ZapIcon,
                title: 'Redis Cache Isolation',
                desc: 'Cache keys prefixed: {tenantId}::{cacheName}::{key} — automatic tenant-scoped scope.',
                color: 'text-pink-400',
              },
            ].map(p => (
        ...
        <section id="security" className="scroll-mt-32">
          <SectionHeader
            ...
            subtitle="JWT, OAuth2, RBAC and granular permissions"
          />
              ...
              <tbody className="divide-border/50 divide-y">
                {[
                  [
                    'SUPER_ADMIN',
                    'All tenants',
                    'Full system · tenant management',
                  ],
                  [
                    'ADMIN',
                    'Own tenant',
                    'All CMS operations · user management',
                  ],
        ...
        <section id="performance" className="scroll-mt-32">
          <SectionHeader
            ...
            subtitle="Before/after metrics and optimization priorities"
          />
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              {
          ...
          <div className="mt-6 grid grid-cols-1 gap-3">
            {[
              {
                p: 'HIGH',
                t: 'N+1 Query Fix with @EntityGraph',
                s: '47 queries → 1 query per request',
                color: 'border-rose-500/30 bg-rose-500/5 text-rose-400',
              },
              {
                p: 'HIGH',
                t: 'HikariCP Pool Size 10 → 50',
                s: 'Prevents connection exhaustion under load',
                color: 'border-rose-500/30 bg-rose-500/5 text-rose-400',
              },
              {
                p: 'HIGH',
                t: 'Database Indexes on FK columns',
                s: 'Eliminates full table scans — 5–10× speed',
                color: 'border-rose-500/30 bg-rose-500/5 text-rose-400',
              },
              {
                p: 'MID',
                t: 'Pagination on all list endpoints',
                s: 'Prevents OOM on large datasets',
                color: 'border-amber-500/30 bg-amber-500/5 text-amber-400',
              },
              {
                p: 'MID',
                t: 'Tenant-prefixed Redis @Cacheable',
                s: '50–100× on cache hits, graceful fallback',
                color: 'border-amber-500/30 bg-amber-500/5 text-amber-400',
              },
              {
                p: 'LOW',
                t: 'Read replica + CDN (file uploads)',
                s: 'Scale read operations horizontally',
                color:
        ...
        <section id="infra" className="scroll-mt-32">
          <SectionHeader
            ...
            subtitle="Kubernetes, observability, mail system, CI/CD"
          />
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {[
              {
              ...
              },
              {
                ...
                color: 'text-pink-400',
                items: [
                  ['Tenant SMTP', 'AES-256 encrypted credentials'],
                  ['Async sending', 'RabbitMQ email-queue'],
                  ['Retry logic', '3× with 30s backoff'],
                  ['Dead letter queue', 'FAILED + DLQ on exhaustion'],
                  ['Verification endpoint', 'SMTP test before send'],
                ],
              },
              {
                title: 'CI/CD & Load Tests',
                color: 'text-amber-400',
                items: [
                  ['GitHub Actions', 'Build → Test → Deploy'],
                  ['Docker', 'Dockerfile + docker-compose'],
                  ['k6 Load Tests', 'baseline · stress · write suites'],
        ...
        <section id="summary" className="scroll-mt-32">
          <SectionHeader
            ...
            subtitle="Summary metrics and next steps"
          />
        ...
        <footer className="border-border/50 mt-40 space-y-2 border-t pb-10 pt-10 text-center text-[10px] text-muted-foreground">
          <p>© {new Date().getFullYear()} Elly CMS — Presentation</p>
          <p>
...
export default PresentationDoc
