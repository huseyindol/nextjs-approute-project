...
## 11. Autovacuum, Bloat ve İstatistik Disiplini

Yüksek yazma hızına sahip tablolarda **transaction ID wraparound** ve **dead tuple** birikimi sessiz performans katilidir. `autovacuum_vacuum_scale_factor` ve `autovacuum_analyze_scale_factor` değerlerini globalde bırakmak yerine, dev tablolar için `ALTER TABLE ... SET (autovacuum_vacuum_scale_factor = 0.05)` gibi tablo bazlı sıkılaştırma yapıyoruz. `pg_stat_all_tables` üzerinden `n_dead_tup` / `n_live_tup` oranı düzenli izlenmeli; `last_autovacuum` tarihi çok geride kaldıysa manuel `VACUUM (ANALYZE)` penceresi planlanır. `pg_stat_statements` ile en pahalı sorguları sabitleyip indeks veya şema düzeltmesine bağlamak, `EXPLAIN`'in tek başına sunduğu anlık görüntüden daha sürdürülebilir bir optimizasyon döngüsü sağlar.

## 12. Rol Bazlı Bağlantı ve En Az Yetki

`max_connections` tavanına çarpmadan önce, uygulama kullanıcılarına `CONNECT` ve ihtiyaç duyulan şema yetkileri dışında hak vermemek; ayrı `readonly_analytics` rolü ile raporlama veya BI trafiğini yazma yolundan ayırmak operasyonel güvenliği güçlendirir. Süper kullanıcı (`postgres`) ile uygulama çalıştırmak production'da kabul edilemez bir anti-pattern'dir.
